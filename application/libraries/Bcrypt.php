...
class Bcrypt
{
    ...
    public function validate($input, $dbsalt, $realpassword)
    {
        $concated_pass = $realpassword . $dbsalt;
        $hashed_input = hash("sha256", $input);
        $concated_input = $hashed_input . $dbsalt;

        return $concated_input === $concated_pass;
    }
    ...
    public function scramble($length = 10)
    {
        $getRandom = $this->getRandomBytes(16);
        $random    = $this->encodeBytes($getRandom);
